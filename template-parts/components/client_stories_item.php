...
<div class="card flex flex-col rounded-2xl shadow-xl bg-white h-full overflow-hidden transition-all hover:shadow-2xl hover:-translate-y-1 group">
    ...
    <div class="pt-14 px-8 pb-8 flex flex-col flex-grow">
        <h3 class="text-2xl font-semibold mb-6 text-gray-900">
            <a href="#<?php echo esc_attr( $modal_id ); ?>" class="no-underline hover:text-purple-600 transition-colors" data-fancybox="stories">
                <?php echo esc_html( $excerpt ? $excerpt : get_the_title() ); ?>
            </a>
        </h3>
        
        <div class="mt-auto">
            <a href="#<?php echo esc_attr( $modal_id ); ?>" class="text-red-600 font-medium tracking-wider uppercase inline-flex items-center hover:text-red-700" data-fancybox="stories">
                <span>Learn More</span>
                <?php echo goodshep_icon( array( 'icon' => 'navigate-right', 'group' => 'utility', 'size' => 10, 'class' => 'inline-block fill-current text-red-600 ml-1' ) ); ?>
            </a>
        </div>
    </div>
...
<div id="<?php echo esc_attr( $modal_id ); ?>" class="hidden w-full max-w-4xl p-0 rounded-xl overflow-hidden bg-white">
    ...
    <div class="p-8 lg:p-12 prose max-w-none">
        <h3 class="text-3xl font-bold text-red-600 mb-4"><?php the_title(); ?></h3>
